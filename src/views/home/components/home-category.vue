<template>
  <div class="home-category" @mouseleave="categoryId=null">
    <ul class="menu">
      <li v-for="i in menuList" :key="i.id" @mouseenter="categoryId = i.id" :class="{active:categoryId === i.id}">
        <router-link :to="`/category/${i.id}`">{{ i.name }}</router-link>
        ...
        <span v-else>
          <skeleton width="60px" hight="18px" style="margin-right: 5px;" bg="rgba(255,255,255,0.2)"></skeleton>
          <skeleton width="50px" hight="18px"  bg="rgba(255,255,255,0.2)"></skeleton>
        </span>
      </li>
    </ul>
    <div class="layer">
...
findBrand().then((res) => {
  if (res.result) {
    brand.brands = res.result.splice(0, 6)
  }
})
